...
# 计算移动平均线 - 3日均线（用于买入和卖出条件）
stock_data['3日均线'] = stock_data[close_price_column].rolling(window=3, min_periods=1).mean()
...
# 确定有效数据范围（均线有值）
valid_data_mask = stock_data['3日均线'].notna()

# 买入条件：收盘价大于3日均线，且前一天收盘价不大于3日均线（避免连续买入）
buy_condition = (stock_data[close_price_column] > stock_data['3日均线']) & \
                (stock_data[close_price_column].shift(1) <= stock_data['3日均线'].shift(1)) & \
                valid_data_mask

# 卖出条件：收盘价低于3日均线，且前一天收盘价不低于3日均线（避免连续卖出）
sell_condition = (stock_data[close_price_column] < stock_data['3日均线']) & \
                 (stock_data[close_price_column].shift(1) >= stock_data['3日均线'].shift(1)) & \
                 valid_data_mask
...
fig.suptitle("K线图与3日均线策略交易信号（含收益率总和）", fontsize=18)
...
# 绘制3日均线
ax1.plot(stock_data.index, stock_data['3日均线'], label='3日均线', color='blue', linewidth=1.5)
...
# 设置K线图属性
ax1.set_title("K线图与交易信号（3日均线策略）", fontsize=14)
